...
function createExecCommand(execCommand, dbPath) {
  execCommand = escapePath(path.join(dbPath, execCommand));
  dbPath = escapePath(dbPath);
  return [execCommand, '--dbpath', dbPath].join(' ');
}
...
function createMongoProcess(execCommand, resolve, reject) {
  ...
  mongoProcess = childProcess.exec(execCommand, function (e, stdout, stderr) {
    if (stdout !== null ) {
...
function start(dbPath, port, noprealloc, nojournal) {
  ...
  if (!!dbPath) {
    execCommand = createExecCommand(execCommand, dbPath);
  }
  if (!!port) {
