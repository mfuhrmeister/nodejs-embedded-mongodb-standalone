...
- [ ] Enforce a redirect limit in `lib/distributer/mongodbDownload.js`
- [ ] Add request timeouts and clearer network failure handling in `lib/distributer/mongodbDownload.js`
- [ ] Reject oversized downloads using `content-length` when available in `lib/distributer/mongodbDownload.js`
- [ ] Remove partial `.in_progress` files on every failure path in `lib/distributer/mongodbDownload.js`
