...
function extract(file, version, extractionBaseDir) {
  return Promise.try(function () {
    ...
    getDecompressFunctionName(file);

    var extractionDir = [(extractionBaseDir) ? extractionBaseDir : os.tmpdir(), DEFAULT_EXTRACTION_DIR, version].join('/');

    return Decompress(file, extractionDir, { strip: 1 })
      .then(function () {
