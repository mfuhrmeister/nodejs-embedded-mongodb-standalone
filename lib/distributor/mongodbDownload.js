import os from 'os';
import https from 'https';
import fs from 'fs';
import path from 'path';
import crypto from 'crypto';
import getos from 'getos';

import { verifySignature } from './gpgVerify.js';
import { SignatureError } from '../error/errors.js';

const DOWNLOAD_BASE_URI = 'https://fastdl.mongodb.org';
const MINIMUM_MACOS_ARM64_MAJOR = 6;
const DEFAULT_REQUEST_TIMEOUT_MS = 30000;
const DEFAULT_MAX_REDIRECTS = 5;
const DEFAULT_MAX_DOWNLOAD_BYTES = 1024 * 1024 * 1024;
const DEFAULT_MAX_TEXT_BYTES = 1024 * 1024;
const ERROR_MESSAGE_TOO_MANY_REDIRECTS = 'download failed: too many redirects';
const ERROR_MESSAGE_CHECKSUM_MISSING = 'checksum file is not available for this MongoDB archive; retry with verify_checksum: false to bypass checksum verification';
const ERROR_MESSAGE_INVALID_VERSION = 'invalid version format; expected semantic version (e.g., 6.0.8)';
const VERSION_REGEX = /^\d+\.\d+\.\d+$/;

function validateVersion(version) {
  if (!VERSION_REGEX.test(version)) {
    throw new Error(ERROR_MESSAGE_INVALID_VERSION);
  }
}

function getVersionMajor(version) {
  const major = Number(String(version).split('.')[0]);

  if (isNaN(major)) {
    return 0;
  }

  return major;
}

function getVersionMinor(version) {
  const minor = Number(String(version).split('.')[1]);

  if (isNaN(minor)) {
    return 0;
  }

  return minor;
}

function getMongoPlatform(platform) {
  switch (platform) {
    case 'darwin':
      return 'osx';
    case 'win32':
      return 'win32';
    case 'linux':
      return 'linux';
    case 'elementary OS':
      return 'linux';
    case 'sunos':
      return 'sunos5';
    default:
      throw new Error('unsupported OS');
  }
}

function getMongoArch(platform, arch) {
  if (arch === 'ia32') {
    if (platform === 'linux') {
      return 'i686';
    }
    if (platform === 'win32') {
      return 'i386';
    }
    throw new Error('unsupported architecture');
  }
  if (arch === 'arm64') {
    if (platform === 'darwin') {
      return 'arm64';
    }
    throw new Error('unsupported architecture');
  }
  if (arch === 'x64') {
    return 'x86_64';
  }
  throw new Error('unsupported architecture, ia32, x64 and darwin arm64 are the only valid options');
}

function getArchiveExtension(mongoPlatform) {
  return mongoPlatform === 'win32' ? 'zip' : 'tgz';
}

function useModernWindowsArchiveLayout(mongoPlatform, version) {
  if (mongoPlatform !== 'win32') {
    return false;
  }

  const majorVersion = getVersionMajor(version);
  const minorVersion = getVersionMinor(version);

  return majorVersion > 4 || (majorVersion === 4 && minorVersion >= 4);
}

function getMongoDownloadPlatform(mongoPlatform, version) {
  if (useModernWindowsArchiveLayout(mongoPlatform, version)) {
    return 'windows';
  }

  return mongoPlatform;
}

function getMongoBaseName(mongoPlatform, mongoArch, version) {
  if (useModernWindowsArchiveLayout(mongoPlatform, version)) {
    return 'mongodb-windows-' + mongoArch;
  }

  if (mongoPlatform !== 'osx') {
    return 'mongodb-' + mongoPlatform + '-' + mongoArch;
  }

  if (mongoArch === 'arm64') {
    if (getVersionMajor(version) < MINIMUM_MACOS_ARM64_MAJOR) {
      throw new Error('unsupported MongoDB version for macOS arm64, use version 6.0.0 or newer');
    }

    return 'mongodb-macos-arm64';
  }

  if (mongoArch === 'x86_64' && getVersionMajor(version) >= MINIMUM_MACOS_ARM64_MAJOR) {
    return 'mongodb-macos-x86_64';
  }

  return 'mongodb-osx-x86_64';
}

function getUbuntuSuffixForVersion(release, version) {
  const majorRelease = Number(String(release).split('.')[0]);
  const majorVersion = getVersionMajor(version);

  if (majorVersion >= 8) {
    if (majorRelease >= 24) {
      return '-ubuntu2404';
    }
    if (majorRelease >= 22) {
      return '-ubuntu2204';
    }
    if (majorRelease >= 20) {
      return '-ubuntu2004';
    }
  }

  if (majorVersion >= 6) {
    if (majorRelease >= 22) {
      return '-ubuntu2204';
    }
    if (majorRelease >= 20) {
      return '-ubuntu2004';
    }
    if (majorRelease >= 18) {
      return '-ubuntu1804';
    }
  }

  if (release === '14.04' || majorRelease > 14) {
    return '-ubuntu1404';
  }
  if (release === '12.04') {
    return '-ubuntu1204';
  }
  if (release === '14.10') {
    return '-ubuntu1410-clang';
  }

  return '-ubuntu1404';
}

function createMongodbDownload(dependencies) {
  const resolvedDependencies = dependencies || {};
  const resolvedOs = resolvedDependencies.os || os;
  const resolvedHttps = resolvedDependencies.https || https;
  const resolvedFs = resolvedDependencies.fs || fs;
  const resolvedPath = resolvedDependencies.path || path;
  const resolvedGetos = resolvedDependencies.getos || getos;

  function getLinuxDistroSuffix(version) {
    return new Promise(function (resolve, reject) {
      resolvedGetos(function (err, osInfo) {
        if (err) {
          reject(err);
          return;
        }
        const dist = (osInfo && osInfo.dist) ? osInfo.dist : '';
        const release = (osInfo && osInfo.release) ? osInfo.release : '';
        let suffix = '';

        if (/ubuntu/i.test(dist)) {
          suffix = getUbuntuSuffixForVersion(release, version);
        } else if (/elementary OS/i.test(dist)) {
          suffix = getUbuntuSuffixForVersion('22.04', version);
        } else if (/suse/i.test(dist)) {
          suffix = '-suse';
          if (/^11/.test(release)) {
            suffix += '11';
          }
        } else if (/rhel/i.test(dist) || /centos/i.test(dist) || /scientific/i.test(dist)) {
          suffix = '-rhel';
          if (/^7/.test(release)) {
            suffix += '70';
          } else if (/^6/.test(release)) {
            suffix += '62';
          } else if (/^5/.test(release)) {
            suffix += '55';
          }
        } else if (/fedora/i.test(dist)) {
          suffix = '-rhel';
          const fedoraVersion = Number(release);
          if (fedoraVersion > 18) {
            suffix += '70';
          } else if (fedoraVersion < 19 && fedoraVersion >= 12) {
            suffix += '62';
          } else if (fedoraVersion < 12 && fedoraVersion >= 6) {
            suffix += '55';
          }
        } else if (/debian/i.test(dist)) {
          suffix = '-debian';
          if (/^(7|8)/.test(release)) {
            suffix += '71';
          }
        }

        resolve(suffix);
      });
    });
  }

  async function exists(filePath) {
    try {
      await resolvedFs.promises.stat(filePath);
      return true;
    } catch (err) {
      if (err && err.code === 'ENOENT') {
        return false;
      }
      throw err;
    }
  }

  function closeStream(stream) {
    return new Promise(function (resolve) {
      if (!stream || typeof stream.close !== 'function') {
        resolve();
        return;
      }

      stream.close(resolve);
    });
  }

  function removeFile(filePath) {
    return resolvedFs.promises.unlink(filePath).catch(function () {
      return;
    });
  }

  function createHttpStatusError(statusCode) {
    const error = new Error('download failed with status ' + statusCode);
    error.statusCode = statusCode;
    return error;
  }

  function renameFile(tempFile, finalFile) {
    return resolvedFs.promises.rename(tempFile, finalFile).then(function () {
      return finalFile;
    });
  }

  function getContentLength(response) {
    const headerValue = response && response.headers ? response.headers['content-length'] : undefined;
    const parsedContentLength = Number(headerValue);

    if (!Number.isInteger(parsedContentLength) || parsedContentLength < 0) {
      return null;
    }

    return parsedContentLength;
  }

  function cleanupTempFile(file, tempFile) {
    return closeStream(file)
      .catch(function () {
        return;
      })
      .then(function () {
        return removeFile(tempFile);
      });
  }

  function downloadText(downloadUrl, httpOpts, downloadState) {
    return new Promise(function (resolve, reject) {
      const requestHttpOpts = Object.assign({}, httpOpts || {});
      const configuredTimeoutMs = typeof requestHttpOpts.timeout === 'number' ? requestHttpOpts.timeout : DEFAULT_REQUEST_TIMEOUT_MS;
      const configuredMaxRedirects = typeof requestHttpOpts.maxRedirects === 'number' ? requestHttpOpts.maxRedirects : DEFAULT_MAX_REDIRECTS;
      const configuredMaxTextBytes = typeof requestHttpOpts.maxTextLength === 'number' ? requestHttpOpts.maxTextLength : DEFAULT_MAX_TEXT_BYTES;
      delete requestHttpOpts.timeout;
      delete requestHttpOpts.maxRedirects;
      delete requestHttpOpts.maxTextLength;
      const state = Object.assign({
        redirectCount: 0,
        maxRedirects: configuredMaxRedirects,
        timeoutMs: configuredTimeoutMs,
        maxTextBytes: configuredMaxTextBytes
      }, downloadState || {});
      const parsed = new URL(downloadUrl);
      const requestOptions = Object.assign({}, requestHttpOpts, {
        protocol: parsed.protocol,
        hostname: parsed.hostname,
        port: parsed.port,
        path: parsed.pathname + parsed.search
      });
      let settled = false;
      let request;
      const chunks = [];
      let totalBytes = 0;

      function rejectOnce(err) {
        if (settled) {
          return;
        }
        settled = true;
        reject(err);
      }

      request = resolvedHttps.get(requestOptions, function (response) {
        if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) {
          if (state.redirectCount >= state.maxRedirects) {
            rejectOnce(new Error(ERROR_MESSAGE_TOO_MANY_REDIRECTS));
            return;
          }

          downloadText(
            new URL(response.headers.location, downloadUrl).toString(),
            httpOpts,
            Object.assign({}, state, {
              redirectCount: state.redirectCount + 1
            })
          ).then(resolve, rejectOnce);
          return;
        }

        if (response.statusCode !== 200) {
          rejectOnce(createHttpStatusError(response.statusCode));
          return;
        }

        const contentLength = getContentLength(response);
        if (contentLength !== null && contentLength > state.maxTextBytes) {
          rejectOnce(new Error('download failed: content-length ' + contentLength + ' exceeds limit ' + state.maxTextBytes));
          return;
        }

        response.on('data', function (chunk) {
          const bufferChunk = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk);
          totalBytes += bufferChunk.length;
          if (totalBytes > state.maxTextBytes) {
            rejectOnce(new Error('download failed: response exceeds limit ' + state.maxTextBytes));
            if (typeof request.destroy === 'function') {
              request.destroy();
            }
            return;
          }
          chunks.push(bufferChunk);
        });

        response.on('end', function () {
          if (settled) {
            return;
          }
          settled = true;
          resolve(Buffer.concat(chunks).toString('utf8'));
        });

        response.on('error', function (err) {
          rejectOnce(err);
        });
      });

      request.on('error', function (err) {
        rejectOnce(err);
      });

      if (typeof request.setTimeout === 'function') {
        request.setTimeout(state.timeoutMs, function () {
          if (typeof request.destroy === 'function') {
            request.destroy();
          }

          rejectOnce(new Error('download timed out after ' + state.timeoutMs + 'ms'));
        });
      }
    });
  }

  function parseSha256(text) {
    const match = String(text).match(/[a-f0-9]{64}/i);
    if (!match) {
      throw new Error('checksum parse failed');
    }
    return match[0].toLowerCase();
  }

  function hashFileSha256(filePath) {
    return new Promise(function (resolve, reject) {
      const hash = crypto.createHash('sha256');
      const stream = resolvedFs.createReadStream(filePath);

      stream.on('data', function (chunk) {
        hash.update(chunk);
      });
      stream.on('error', function (err) {
        reject(err);
      });
      stream.on('end', function () {
        resolve(hash.digest('hex'));
      });
    });
  }

  async function verifyChecksum(downloadUrl, filePath, httpOpts) {
    const checksumUrl = downloadUrl + '.sha256';
    let checksumText;

    try {
      checksumText = await downloadText(checksumUrl, httpOpts);
    } catch (err) {
      if (err && err.statusCode === 404) {
        throw new Error(ERROR_MESSAGE_CHECKSUM_MISSING);
      }
      throw err;
    }

    const expected = parseSha256(checksumText);
    const actual = await hashFileSha256(filePath);

    if (expected !== actual) {
      throw new Error('checksum mismatch for ' + path.basename(filePath));
    }
  }

  function downloadToFile(downloadUrl, tempFile, finalFile, httpOpts, downloadState) {
    return new Promise(function (resolve, reject) {
      const requestHttpOpts = Object.assign({}, httpOpts || {});
      const configuredTimeoutMs = typeof requestHttpOpts.timeout === 'number' ? requestHttpOpts.timeout : DEFAULT_REQUEST_TIMEOUT_MS;
      const configuredMaxRedirects = typeof requestHttpOpts.maxRedirects === 'number' ? requestHttpOpts.maxRedirects : DEFAULT_MAX_REDIRECTS;
      const configuredMaxDownloadBytes = typeof requestHttpOpts.maxContentLength === 'number'
        ? requestHttpOpts.maxContentLength
        : DEFAULT_MAX_DOWNLOAD_BYTES;
      delete requestHttpOpts.timeout;
      delete requestHttpOpts.maxRedirects;
      delete requestHttpOpts.maxContentLength;
      const state = Object.assign({
        redirectCount: 0,
        maxRedirects: configuredMaxRedirects,
        timeoutMs: configuredTimeoutMs,
        maxDownloadBytes: configuredMaxDownloadBytes
      }, downloadState || {});
      const parsed = new URL(downloadUrl);
      const requestOptions = Object.assign({}, requestHttpOpts, {
        protocol: parsed.protocol,
        hostname: parsed.hostname,
        port: parsed.port,
        path: parsed.pathname + parsed.search
      });
      let settled = false;
      let request;

      const file = resolvedFs.createWriteStream(tempFile);

      function rejectWithCleanup(err) {
        if (settled) {
          return;
        }

        settled = true;
        cleanupTempFile(file, tempFile).then(function () {
          reject(err);
        }, function () {
          reject(err);
        });
      }

      request = resolvedHttps.get(requestOptions, function (response) {
        if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) {
          if (state.redirectCount >= state.maxRedirects) {
            rejectWithCleanup(new Error(ERROR_MESSAGE_TOO_MANY_REDIRECTS));
            return;
          }

          cleanupTempFile(file, tempFile)
            .then(function () {
              return downloadToFile(
                new URL(response.headers.location, downloadUrl).toString(),
                tempFile,
                finalFile,
                httpOpts,
                Object.assign({}, state, {
                  redirectCount: state.redirectCount + 1
                })
              );
            })
            .then(resolve, reject);
          return;
        }

        if (response.statusCode !== 200) {
          rejectWithCleanup(createHttpStatusError(response.statusCode));
          return;
        }

        const contentLength = getContentLength(response);
        if (contentLength !== null && contentLength > state.maxDownloadBytes) {
          rejectWithCleanup(new Error('download failed: content-length ' + contentLength + ' exceeds limit ' + state.maxDownloadBytes));
          return;
        }

        response.pipe(file);

        file.on('finish', function () {
          closeStream(file)
            .then(function () {
              return renameFile(tempFile, finalFile);
            })
            .then(resolve, rejectWithCleanup);
        });

        response.on('error', function (err) {
          rejectWithCleanup(err);
        });
      });

      file.on('error', function (err) {
        rejectWithCleanup(err);
      });

      request.on('error', function (err) {
        rejectWithCleanup(err);
      });

      if (typeof request.setTimeout === 'function') {
        request.setTimeout(state.timeoutMs, function () {
          if (typeof request.destroy === 'function') {
            request.destroy();
          }

          rejectWithCleanup(new Error('download timed out after ' + state.timeoutMs + 'ms'));
        });
      }
    });
  }

  async function mongodbDownload(opts) {
    const options = opts || {};

    const platform = options.platform || resolvedOs.platform();
    const mongoPlatform = getMongoPlatform(platform);

    const arch = options.arch || resolvedOs.arch();
    const mongoArch = getMongoArch(platform, arch);

    const version = options.version;
    if (!version) {
      throw new Error('missing version');
    }

    validateVersion(version);

    const archiveExt = getArchiveExtension(mongoPlatform);
    const downloadPlatform = getMongoDownloadPlatform(mongoPlatform, version);
    const baseName = getMongoBaseName(mongoPlatform, mongoArch, version);

    const linuxSuffix = (mongoPlatform === 'linux' && mongoArch !== 'i686') ? await getLinuxDistroSuffix(version) : '';
    const fileName = baseName + linuxSuffix + '-' + version + '.' + archiveExt;
    const downloadUrl = DOWNLOAD_BASE_URI + '/' + downloadPlatform + '/' + fileName;

    const tempDir = options.download_dir || resolvedOs.tmpdir();
    const downloadDir = resolvedPath.resolve(tempDir, 'mongodb-download');
    const finalFile = resolvedPath.resolve(downloadDir, fileName);
    const tempFile = resolvedPath.resolve(downloadDir, fileName + '.in_progress');

    await resolvedFs.promises.mkdir(downloadDir, { recursive: true });

    if (await exists(finalFile)) {
      return finalFile;
    }

    const downloadedFile = await downloadToFile(downloadUrl, tempFile, finalFile, options.http_opts);
    const verifyChecksumEnabled = options.verify_checksum !== false;
    const verifySignatureEnabled = options.verify_signature === 'gpg' || options.verify_signature === 'both';

    if (!verifyChecksumEnabled && !verifySignatureEnabled) {
      return downloadedFile;
    }

    try {
      if (verifySignatureEnabled) {
        await verifySignature(downloadedFile, version, options.http_opts);
      }

      if (verifyChecksumEnabled) {
        await verifyChecksum(downloadUrl, downloadedFile, options.http_opts);
      }

      return downloadedFile;
    } catch (err) {
      await removeFile(downloadedFile);
      if (err.message && err.message.includes('signature')) {
        throw new SignatureError(err.message);
      }
      throw err;
    }
  }

  mongodbDownload.downloadToFile = downloadToFile;
  mongodbDownload.getLinuxDistroSuffix = getLinuxDistroSuffix;

  return mongodbDownload;
}

const mongodbDownload = createMongodbDownload();

const getLinuxDistroSuffix = mongodbDownload.getLinuxDistroSuffix;
const downloadToFile = mongodbDownload.downloadToFile;

export {
  createMongodbDownload,
  getLinuxDistroSuffix,
  downloadToFile,
  getVersionMajor,
  getVersionMinor,
  getMongoPlatform,
  getMongoDownloadPlatform,
  getMongoArch,
  getArchiveExtension,
  getMongoBaseName
};
export default mongodbDownload;
